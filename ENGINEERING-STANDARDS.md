...
(believed at the time to be `num_queries * log2(blowup_factor) +
grinding_factor`) worked out to ~96 bits (32 queries, blowup 16, grinding
0, no field extension), and `NovaRescue`'s round count was 7. **That
formula was only part of winterfell's own, and both the before and after
numbers in this section are corrected in §6.30 below** — the real
pre-hardening figure was 63 bits, not 96. Neither was
...
FieldExtension::Quadratic` (~148 bits by the same formula — really 127,
see §6.30; the quadratic extension separately addressing winterfell's own
documented caveat that a field this size can fall short of 100+ bits of
